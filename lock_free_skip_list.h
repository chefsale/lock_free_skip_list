...
class Node {
  public:
    ...
    Node(V value)
      : value_(value), height_(max_level_) {
      key_ = value;
      next_.resize(max_level_, new std::atomic<MarkableReference<Node<V>>>(nullptr));
    }
    Node(V value, int height)
      : value_(value), height_(height) {
      key_ = value;
      ...
      while (true) {
        ...
        if (found) {
          ...
          if (!pred->next_[0]->compare_exchange_strong(
            x,
            MarkableReference<Node<V>>(new_node))) continue;
          break;  
        for (int level = 1; level <= top_level; level++) {
          while (true) {
            ...
            if (pred->next_[level]->compare_exchange_strong(
              x,
              MarkableReference<Node<V>>(new_node))) break;
      ...
      if (!found) return false;
        ...
        for (int level = remove_node->height_-1; level >= 0; level--) {
          ...
          bool marked = remove_node->next_[level]->load().getMark();
            auto x =
MarkableReference<Node<V>>(remove_node->next_[level]->load().getRef());
          while (!marked) {
            std::cout << "Marking" << std::endl;
            auto y =
MarkableReference<Node<V>>(remove_node->next_[level]->load().getRef(), true);
            remove_node->next_[level]->compare_exchange_strong(
              x,
              y); 
        ...
        while (true) {
          auto x =
MarkableReference<Node<V>>(remove_node->next_[0]->load().getRef(), false);
          auto y =
MarkableReference<Node<V>>(remove_node->next_[0]->load().getRef(), true);
          bool i_marked_it =  
